...
exports.setup = function (app) {
  ...
  app.post('/expectRequest', function (req, res) {
    ...
    app[method](expectConfig.url, function (req, res, next) {
      var expectConfigAsString = JSON.stringify(expectConfig);
      receivedRequestCountByExpectConfig[expectConfigAsString] = receivedRequestCountByExpectConfig[expectConfigAsString] || 0;
      receivedRequestCountByExpectConfig[expectConfigAsString]++;
      next();
    });
  ...
  app.post('/hasReceived', function (req, res) {
    var expectConfig = req.body.expectConfig;
    var expectConfigAsString = JSON.stringify(expectConfig);
    var count = receivedRequestCountByExpectConfig[expectConfigAsString] || 0;
    res.set('Cache-Control', 'no-cache');
